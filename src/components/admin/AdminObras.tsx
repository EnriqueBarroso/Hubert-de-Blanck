...
import { Plus, Pencil, ChevronRight, Users, Trash2 } from 'lucide-react';
...
export default function AdminObras() {
  ...
  async function guardarObra(e: FormEvent) {
    ...
    // --- SOLUCIÓN: Extraer producciones para que no se envíen a la tabla 'obras' ---
  ...
  return (
    <div>
          ...
          {obras.map((obra) => {
            ...
            return (
              <div key={obra.id} className="border-2 border-ink bg-paper-2">
                ...
                {/* Producciones (expandido) */}
                {abierta && (
                  <div className="border-t border-ink/30 bg-paper px-4 py-4">
                    <div className="flex justify-between items-baseline mb-3">
                      <span className="font-mono text-[10px] tracking-widest uppercase text-ink/60">
                        Producciones de esta obra
                      </span>
                      <button
      ...
      {obraEditando && (
        <Modal
          abierto={!!obraEditando}
          onCerrar={() => setObraEditando(null)}
          titulo={obraEditando.id ? 'Editar obra' : 'Nueva obra'}
          maxAncho="md"
        >
          <form onSubmit={guardarObra} className="space-y-4">
            <div className="flex flex-col sm:flex-row gap-4 items-start">
              <ImageUpload
                valor={obraEditando.foto_portada_url ?? null}
                onCambio={(url) => actualizarObra('foto_portada_url', url)}
                carpeta="obras"
                aspecto="retrato"
                label="Portada"
              />
              <div className="flex-1 w-full space-y-3">
                <FormField label="Título" requerido>
                  <input
                    ...
                    onChange={(e) => actualizarObra('titulo', e.target.value)}
                    onBlur={() => {
                      if (!obraEditando.slug && obraEditando.titulo) {
                        actualizarObra('slug', slugify(obraEditando.titulo));
                      }
                    }}
                ...
                <FormField label="Slug (URL)" helper="Se genera del título.">
                  <input
                    type="text"
                    value={obraEditando.slug ?? ''}
                    onChange={(e) => actualizarObra('slug', slugify(e.target.value))}
                ...
                <FormField label="Autor">
                  <input
                    type="text"
                    value={obraEditando.autor ?? ''}
                    onChange={(e) => actualizarObra('autor', e.target.value || null)}
                    className="input-brut"
                  />
                </FormField>
              </div>
            ...
            <FormField label="Sinopsis">
              <textarea
                value={obraEditando.sinopsis ?? ''}
                onChange={(e) => actualizarObra('sinopsis', e.target.value || null)}
                rows={3}
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              <FormField label="Año estreno">
                <input
                  type="number"
                  value={obraEditando.anio_estreno ?? ''}
                  onChange={(e) => actualizarObra('anio_estreno', e.target.value ? Number(e.target.value) : null)}
                  className="input-brut"
                />
              </FormField>
              <FormField label="Duración (min)">
                <input
                  type="number"
                  value={obraEditando.duracion_minutos ?? ''}
                  onChange={(e) => actualizarObra('duracion_minutos', e.target.value ? Number(e.target.value) : null)}
                  className="input-brut"
                />
              </FormField>
              <FormField label="Estado" requerido>
                <select
                  value={obraEditando.estado ?? 'archivada'}
                  onChange={(e) => actualizarObra('estado', e.target.value as EstadoObra)}
                  className="input-brut"
                >
            ...
            <FormField label="Director general (opcional)">
              <select
                value={obraEditando.director_id ?? ''}
                onChange={(e) => actualizarObra('director_id', e.target.value || null)}
                className="input-brut"
              >
      ...
      {prodEditando && (
        <Modal
          abierto={!!prodEditando}
          onCerrar={() => setProdEditando(null)}
          titulo={prodEditando.prod.id ? 'Editar producción' : 'Nueva producción'}
          maxAncho="sm"
        >
          <form onSubmit={guardarProd} className="space-y-4">
            <FormField label="Temporada / nombre" requerido>
              <input
                type="text"
                value={prodEditando.prod.temporada ?? ''}
                onChange={(e) => actualizarProd('temporada', e.target.value)}
                className="input-brut"
            ...
            <div className="grid grid-cols-2 gap-3">
              <FormField label="Año" requerido>
                <input
                  type="number"
                  value={prodEditando.prod.anio ?? ''}
                  onChange={(e) => actualizarProd('anio', Number(e.target.value))}
                  className="input-brut"
                  required
                />
              </FormField>
              <FormField label="Estado">
                <select
                  value={prodEditando.prod.estado ?? 'archivada'}
                  onChange={(e) => actualizarProd('estado', e.target.value as EstadoObra)}
                  className="input-brut"
                >
                  {Object.entries(ESTADO_LABELS).map(([k, v]) => (
                    <option key={k} value={k}>{v}</option>
            ...
            <FormField label="Director (si difiere del general)">
              <select
                value={prodEditando.prod.director_id ?? ''}
                onChange={(e) => actualizarProd('director_id', e.target.value || null)}
                className="input-brut"
              >
            ...
            <FormField label="Notas internas">
              <textarea
                value={prodEditando.prod.notas ?? ''}
                onChange={(e) => actualizarProd('notas', e.target.value || null)}
                rows={2}
...
// ─── Editor de reparto (dentro del mismo archivo para mantener cohesión) ────
